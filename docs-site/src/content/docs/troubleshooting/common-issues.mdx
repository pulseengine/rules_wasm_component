...
### "No repository visible as '@crates'" (rust_wasm_component_bindgen)

**Symptom**:

```
ERROR: no such package '@@[unknown repo 'crates' requested from @@]//':
The repository '@@[unknown repo 'crates' requested from @@]' could not be resolved:
No repository visible as '@crates' from main repository
```

**Cause**: When using `rust_wasm_component_bindgen` from an external workspace, the `@crates` repository that rules_wasm_component uses internally is not visible to your workspace due to bzlmod isolation.

**Solution**: Provide your own bitflags crate via the `bitflags_dep` attribute:
...
Make sure bitflags is in your Cargo.toml:
...
And crate_universe is configured:
...
// ❌ Wrong - looks for world.wit in src/
...
world hello {
    ...
    // ❌ This would be an import (function you need from outside)
...
# ❌ Don't use regular WASM rules for components
# rust_binary(
#     name = "my_component",
#     srcs = ["src/lib.rs"],
#     crate_type = "cdylib",
...
```python title="BUILD.bazel"
# ✅ Use unique names for each target
wit_library(
...
# ❌ Don't use: C:\My Project\components
# ✅ Use: C:\projects\my-components
```
...
### "Undefined symbol: \_\_cxa_allocate_exception"
...
# ❌ Avoid version ranges that might conflict
# wit-bindgen = "^0.30"
```
...
Remember: Most issues are caused by missing dependencies, incorrect file paths, or typos in configuration. Double-check the basics first! 🔍
