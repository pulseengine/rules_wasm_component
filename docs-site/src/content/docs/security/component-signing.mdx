...
> **📋 Rule Reference:** For complete details on all signing and verification rule attributes, see [`wasm_keygen`](/reference/rules/#wasm_keygen), [`wasm_sign`](/reference/rules/#wasm_sign), [`wasm_verify`](/reference/rules/#wasm_verify), and [`wasm_validate`](/reference/rules/#wasm_validate).
...
wasmsign2 verify component.wasm --public-key component.pub
if [ $? -eq 0 ]; then
    echo "✅ Component signature valid"
else
    echo "❌ Component signature invalid"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    echo "✅ OCI image signature valid"
else
    echo "❌ OCI image signature invalid"
    exit 1
fi
...
### Signing Components with Embedded AOT Artifacts

**WebAssembly components with embedded AOT artifacts maintain full signing compatibility** since they remain in the WebAssembly format. This enables universal deployment with both performance optimization and security verification.

#### AOT Embedding + Signing Workflow

```python
load("@rules_wasm_component//wasm:defs.bzl",
     "wasm_precompile", "wasm_embed_aot", "wasm_sign")

# Create AOT artifacts for multiple architectures
wasm_precompile(
    name = "service_x64_aot",
...
wasm_precompile(
    name = "service_portable_aot",
    component = ":my_service",
    target_triple = "pulley64",  # Architecture-independent fallback
    optimization_level = "2",
)
...
wasm_embed_aot(
    ...
    component = ":my_service",
    aot_artifacts = {
        "linux_x64": ":service_x64_aot",
        "linux_arm64": ":service_arm64_aot",
        "portable": ":service_portable_aot",
    },
)

# Sign the universal component (protects component + all embedded AOT artifacts)
wasm_sign(
    name = "signed_universal_service",
...
#### Security Benefits of AOT Embedding + Signing

- **Complete package protection** - Single signature covers WebAssembly component and all embedded native code
- **Architecture-specific verification** - Each platform gets verified optimized code
- **Universal distribution** - One signed artifact works on all supported architectures
- **Fallback security** - Portable bytecode provides compatibility with signature verification
- **Performance + security** - 87% size reduction with cryptographic integrity

#### Verification of Embedded AOT Components

```python
# Verify the universal component (validates signature for entire package)
wasm_verify(
    name = "verify_universal_service",
    signed_component = ":signed_universal_service",
    keys = ":trusted_production_keys",
)

# Extract architecture-specific artifacts after verification
wasm_extract_aot(
    name = "verified_x64_service",
    component = ":signed_universal_service",  # Pre-verified
    target_name = "linux_x64",
)
```

**Security guarantee:** The signature verification covers the entire embedded package, so extracted AOT artifacts inherit the trust of the verified universal component.
...
# Push signed universal component with embedded AOT
wasm_component_oci_publish(
    name = "push_universal_signed",
    component = ":signed_universal_service",
    registry = "ghcr.io",
    namespace = "my-org",
    component_name = "universal-service",
    tag = "v1.0.0",
    # Universal component with signature covers all architectures
)
```
