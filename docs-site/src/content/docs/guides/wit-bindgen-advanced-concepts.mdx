...
flowchart LR
    ...
    R1 --> P1[Heap Allocation<br/>✅ Simple<br/>❌ Performance]
    R2 --> P2[Zero-Copy<br/>✅ Fast<br/>❌ Lifetime complexity]
    R3 --> P3[Flexible<br/>✅ Adaptive<br/>⚠️ Runtime overhead]
```
...
flowchart TB
    ...
    GB --> C1[String ↔ UTF-8 bytes]
    GB --> C2[BigInt ↔ u64/i64]
    GB --> C3[ArrayBuffer ↔ list~u8~]
    GB --> C4[undefined ↔ option~T~]
```
...
flowchart TD
    subgraph "Compilation Impact"
        D[Derives Added] --> CT[Compile Time ↑]
        D --> BS[Binary Size ↑]
        D --> DC[Dependencies ↑]
    end
