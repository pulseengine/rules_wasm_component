...
function generateRuleReference(schemas) {
  ...
  Object.entries(schemas).forEach(([name, data]) => {
    if (data.type === 'provider') {
      ...
      name.includes('wasm_component_new') ||
      name.includes('wasm_component_wizer') ||
      name.includes('wasm_validate')
    ) {
      categories['Component Tools'].push({ name, data });
    } else if (
      name.includes('wasm_sign') ||
      name.includes('wasm_verify') ||
      name.includes('wasm_keygen')
    ) {
