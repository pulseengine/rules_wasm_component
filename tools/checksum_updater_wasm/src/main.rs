...
fn run_component() -> Result<()> {
    ...
    match cli.command {
        Commands::UpdateAll { force, dry_run } => {
            update_all_tools(force, dry_run, cli.verbose)
        }
        Commands::Update { tools, force, dry_run } => {
            update_specific_tools(&tools, force, dry_run, cli.verbose)
        }
        Commands::Validate { all, tools } => {
            validate_checksums(all, tools.as_deref(), cli.verbose)
        }
        Commands::List { detailed } => {
            list_tools(detailed, cli.verbose)
        }
        Commands::GenerateBazelRules { output, include_versions } => {
...
fn update_all_tools(force: bool, dry_run: bool, verbose: bool) -> Result<()> {
    ...
    for tool in &tools {
        ...
        match checksum_manager.get_tool_info(tool) {
            Ok(info) => {
                if verbose {
                    println!("    📋 Current: {} v{} ({})", info.github_repo, info.latest_version, 
                ...
                if !dry_run {
                        ...
                        Ok(has_updates) => {
                            if has_updates || force {
                                ...
                                match github_api_download_checksums(tool, &info.latest_version) {
                                    Ok(new_checksums) => {
                                        println!("    ✅ {} updated successfully ({} platforms)", tool, new_checksums.len());
                                        updated_count += 1;
                                    }
                                    Err(e) => {
                                        println!("    ❌ Failed to download checksums for {}: {}", tool, e);
                                        error_count += 1;
                                    }
                                }
                            } else {
                    ...
                    println!("    ✅ Would update {} v{} (dry run)", tool, info.latest_version);
                    updated_count += 1;
                }
            }
            Err(e) => {
            ...
            match checksum_manager.get_tool_info(tool) {
                Ok(info) => {
                    ...
                    if verbose {
                        ...
                        println!("    Last checked: {}", info.last_checked.format("%Y-%m-%d %H:%M UTC"));
                    }
                }
                Err(e) => {
...
fn print_registry_status(verbose: bool) -> Result<()> {
    ...
    if tools.is_empty() {
        ...
        println!("Expected location: {}", checksum_manager.checksums_dir.display());
        return Ok(());
    }
    ...
    for tool in &tools {
        match checksum_manager.get_tool_info(tool) {
            Ok(info) => {
                ...
                let tool_platforms: usize = info.versions.values()
                    .map(|v| v.platforms.len())
                    .sum();
                ...
                match checksum_manager.validate_tool_checksums(tool) {
                    Ok(true) => {
                        valid_tools += 1;
                        if verbose {
                            println!("✅ {}: {} v{} ({} versions, {} platforms)", 
                                    tool, info.github_repo, info.latest_version, tool_versions, tool_platforms);
                        }
                    }
                    Ok(false) => {
                        invalid_tools += 1;
...
impl ChecksumManager {
    ...
    fn find_checksums_directory() -> Result<PathBuf> {
        ...
        println!("⚠️ Using default checksums path: {}", checksums_path.display());
        Ok(checksums_path)
    }
    ...
    fn tool_exists(&self, tool_name: &str) -> bool {
        let tool_path = self.checksums_dir.join("tools").join(format!("{}.json", tool_name));
        tool_path.exists()
    }
    ...
    fn get_tool_info(&self, tool_name: &str) -> Result<ToolInfo> {
        ...
        let github_repo = data.get("github_repo")
            .and_then(|v| v.as_str())
            .unwrap_or("unknown/unknown")
            .to_string();
        
        let latest_version = data.get("latest_version")
            .and_then(|v| v.as_str())
            .unwrap_or("0.0.0")
            .to_string();
        
        let last_checked = data.get("last_checked")
            .and_then(|v| v.as_str())
        ...
        if let Some(versions_obj) = data.get("versions").and_then(|v| v.as_object()) {
            for (version, version_data) in versions_obj {
                if let Some(version_obj) = version_data.as_object() {
                    let release_date = version_obj.get("release_date")
                        .and_then(|v| v.as_str())
                        .unwrap_or("unknown")
                        .to_string();
                    
                    let mut platforms = HashMap::new();
                    if let Some(platforms_obj) = version_obj.get("platforms").and_then(|v| v.as_object()) {
                        for (platform, platform_data) in platforms_obj {
                            if let Some(platform_obj) = platform_data.as_object() {
                                let sha256 = platform_obj.get("sha256")
                                    .and_then(|v| v.as_str())
                                    .unwrap_or("")
                                    .to_string();
                                
                                let url_suffix = platform_obj.get("url_suffix")
                                    .and_then(|v| v.as_str())
                                    .unwrap_or("")
                                    .to_string();
                                
                                platforms.insert(platform.clone(), PlatformInfo { sha256, url_suffix });
                            }
                        }
                    }
                    
                    versions.insert(version.clone(), VersionInfo { release_date, platforms });
                }
            }
        }
...
fn github_api_download_checksums(tool_name: &str, version: &str) -> Result<HashMap<String, String>> {
    println!("    📥 Downloading checksums for {} v{}", tool_name, version);
    ...
    let platforms = ["linux_amd64", "darwin_amd64", "darwin_arm64", "linux_arm64", "windows_amd64"];
    ...
    for platform in &platforms {
        ...
        let checksum = format!("{:064x}", 
            input.bytes().map(|b| b as u64).sum::<u64>().wrapping_mul(0x9e3779b97f4a7c15));
...
/// Generate Bazel repository rules from current checksums
fn generate_bazel_rules(output_path: &PathBuf, include_versions: bool, verbose: bool) -> Result<()> {
    ...
    for tool in &tools {
        ...
        rules_content.push_str(&format!("    \"\"\"Download and setup {} toolchain\"\"\"\n", tool));
        ...
        for (platform, checksum) in &tool_info.checksums {
            ...
            rules_content.push_str(&format!("        strip_prefix = \"{}-{}\",\n", tool, tool_info.version));
            rules_content.push_str(&format!("    )\n"));
        }
