...
class WitValidator:
    ...
    def extract_component_wit(self, component_path: str) -> Optional[str]:
        ...
        try:
            ...
            if result.returncode != 0:
                print(f"Error extracting component WIT: {result.stderr}", file=sys.stderr)
                return None
    ...
    def validate_exports(self, component_path: str, expected_wit_path: str, world_name: str) -> bool:
        ...
        for interface_name, expected_functions in expected_exports.items():
            ...
            if not component_functions:
                print(f"  ❌ Interface '{interface_name}' not found in component exports")
...
def main():
    parser = argparse.ArgumentParser(description="Validate WebAssembly component WIT exports")
