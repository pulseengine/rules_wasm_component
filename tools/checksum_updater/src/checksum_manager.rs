...
mod tests {
    ...
    #[tokio::test]
    async fn test_create_and_get_tool() {
        ...
        let retrieved = manager.get_tool_info("test-tool").await.unwrap();
    ...
    async fn test_list_tools() {
        ...
        manager.create_tool("tool1", "owner/tool1").await.unwrap();
        manager.create_tool("tool2", "owner/tool2").await.unwrap();

        let tools = manager.list_all_tools().await.unwrap();
    ...
    async fn test_update_tool_version() {
        ...
        manager
            .update_tool_version("test-tool", "1.0.0", version_info)
            .await
            .unwrap();

        let tool_info = manager.get_tool_info("test-tool").await.unwrap();
        assert_eq!(tool_info.latest_version, "1.0.0");
        assert!(tool_info.versions.contains_key("1.0.0"));
    }
}
